...
<main class="flex-shrink-0">
            ...
            <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
                <div class="container px-5">
                    <a class="navbar-brand" href="{{URL('redirect')}}">Healthymind/<i>Blog</i></a>
                    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation"><span class="navbar-toggler-icon"></span></button>
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                        
                            <li class="nav-item"><a class="nav-link" href="{{URL('/blog')}}">Blog</a></li>
                            <li class="nav-item"><a class="nav-link" href="{{URL('/myposts')}}">Myposts</a></li>
...
<header class="mb-4">
<h1 style="font-size:40px;" class="fw-bolder m-5 ml-5 mb-5">Welcome <u><i>{{Auth::user()->name}}</i></u> ! Check out all your created posts!</h1>
</header>
<div  style="">
<form class="form-inline" action="{{URL('searchmyposts')}}" style="padding:20px;" type="get">
@csrf 
...
@if ($posts->role == 'editor')
   ...
   @endif
                                        ...
                                        <a href="/blog/{{ $posts->slug }}/edit" class="m-2 badge bg-success">Edit</a>
                                     </form>
                                        <form action="/blog/{{ $posts->slug }}" method="POST" >
                                        @csrf
                                   @method('delete')
                                        <button type="submit" class="m-2 badge bg-danger">Delete</button>
                                     </form>
                                     @endif
                                    </div>
                                </div>
