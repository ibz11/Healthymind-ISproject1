...
<form class="form-inline" action="{{URL('searchalltherapists')}}" style="padding:20px;" type="get">
@csrf 
<input type="search" name="search">
<button type="submit"   style="color:black;"class="ml-2 btn btn-primary"  >Search</button>
<a href="{{URL('alltherapists' )}}" class="m-1 btn btn-info">Refresh</a>
</form>
...
@if($user->role == 'therapist')
    <tr>
      ...
      <td>{{$user->email}}</td>
      <td>{{$user->phone}}</td>
      <td><i class="fw-bold badge bg-info">{{$user->role}}</i></td>
