...
<h1 class="mt-4 text-2xl"><strong>Update Reply</strong></h1>

<form action="{{URL('updatereply',$reply->id)}}" method="POST" id="editform" enctype="multipart/form-data">
    @csrf
...
<div  class="mb-3">
    ...
    <input   type="text" class="form-control w-50" id="name" value="{{$reply->user_name}}" name="user_name">
    @else
    <label for="pwd" style="font-size:22px;"class="form-label"><strong>Replier's name</strong></label></br>
    <label for=""class="text-muted">Nb:You can change the name since you are not the creator of this reply</label>
    <input  readonly type="text" class="form-control w-50" id="name" value="{{$reply->user_name}}" name="user_name">
    @endif
</div>
...
<div class="mb-3">
  ...
  <textarea name="reply"  name="long"id="" cols="30" rows="10">
  {{$reply->reply}}
  </textarea>
</div>
